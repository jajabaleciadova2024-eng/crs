      ...
      <div className={`flex ${isOwn ? "justify-end" : "justify-start"}`}>
        <div className={`flex items-end gap-1.5 max-w-[80%] ${isOwn ? "flex-row-reverse" : ""}`}>
          ...
          <div className={`min-w-0 ${isOwn ? "items-end" : "items-start"} flex flex-col`}>
            ...
            <div
              ref={actionsRef}
              onClick={() => { if (window.matchMedia("(hover: none)").matches && !editing) setShowActions((v) => !v); }}
            ...
            >
              {editing ? (
                <div className="flex flex-col gap-1.5 min-w-[200px]">
                  ...
                  <div className="flex gap-1.5 justify-end">
                    <button type="button" onClick={() => setEditing(false)} aria-label="Cancel editing" className="text-[10px] px-2 py-0.5 rounded-md hover:bg-white/20 cursor-pointer transition-colors">Cancel</button>
                    <button type="button" onClick={handleSaveEdit} aria-label="Save edit" className="text-[10px] px-2 py-0.5 rounded-md bg-white/20 hover:bg-white/30 font-bold cursor-pointer transition-colors">Save</button>
                  </div>
                </div>
              ) : (
                <>
              ...
              {/* Hover action menu */}
              {!editing && (
                <div className={`absolute ${isOwn ? "-left-1 -translate-x-full" : "-right-1 translate-x-full"} top-0 ${showActions ? "opacity-100" : "opacity-0"} group-hover:opacity-100 focus-within:opacity-100 transition-opacity flex items-center gap-0.5 z-10`}>
                  <button
                    type="button"
                    onClick={() => setShowReactions(!showReactions)}
                    aria-label="React"
                    className="w-7 h-7 rounded-full bg-[var(--paper-raised)] border border-[var(--line)] text-[var(--muted)] hover:text-[var(--accent-strong)] flex items-center justify-center text-[13px] cursor-pointer transition-colors"
                  ...
                  <button
                    ...
                    onClick={onReply}
                    aria-label="Reply"
                    className="w-7 h-7 rounded-full bg-[var(--paper-raised)] border border-[var(--line)] text-[var(--muted)] hover:text-[var(--accent-strong)] flex items-center justify-center cursor-pointer transition-colors"
                    style={{ boxShadow: "var(--shadow-xs)" }}
                  >
                  ...
                  {isTL && (
                    <button
                      ...
                      className="w-7 h-7 rounded-full bg-[var(--paper-raised)] border border-[var(--line)] text-[var(--muted)] hover:text-[var(--bad)] flex items-center justify-center cursor-pointer transition-colors"
                      style={{ boxShadow: "var(--shadow-xs)" }}
                    >
                      <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
              ...
              <div className={`flex flex-wrap gap-1 mt-0.5 px-1 ${isOwn ? "justify-end" : "justify-start"}`}>
                {Object.entries(reactionGroups).map(([emoji, { count, names, hasMine }]) => (
                  <button
                    ...
                    aria-label={`${emoji} reaction by ${names.join(", ")}`}
